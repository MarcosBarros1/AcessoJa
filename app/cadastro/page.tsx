...
import { CheckCircle, Accessibility } from 'lucide-react'; // 🌟 Adicionado ícone de Acessibilidade
...
export default function CadastroCurso() {
  ...
  // 🌟 EFEITOS DE ACESSIBILIDADE (Fonte e Contraste salvos)
  useEffect(() => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 flex flex-col ${altoContraste ? 'bg-black text-yellow-300' : 'bg-slate-50 text-gray-900'}`}>
      
      {/* 🌟 CABEÇALHO DE ACESSIBILIDADE */}
      <header className={`sticky top-0 z-40 backdrop-blur-md border-b-2 shadow-sm px-6 py-4 flex justify-between items-center flex-wrap gap-4 ${altoContraste ? 'bg-black/90 border-yellow-300' : 'bg-white/90 border-slate-200'}`}>
        <div className="flex items-center gap-3 text-2xl font-black tracking-tight text-blue-700">
      ...
      <main className="relative flex flex-col items-center justify-center flex-grow p-4 sm:p-8">
        ...
        <div className={`w-full max-w-3xl p-8 border border-t-8 shadow-xl sm:p-12 rounded-2xl ${altoContraste ? 'bg-black border-yellow-300 border-t-yellow-300' : 'bg-white border-gray-200 border-t-blue-700'}`}>
          
          <header className="flex flex-col gap-4 mb-8 sm:flex-row sm:justify-between sm:items-center">
            <div>
              <h1 className={`text-3xl font-extrabold tracking-tight sm:text-4xl ${altoContraste ? 'text-yellow-300' : 'text-gray-900'}`}>Cadastrar Curso</h1>
              <p className={`mt-2 text-lg ${altoContraste ? 'text-yellow-100' : 'text-gray-600'}`}>Adicione um novo conteúdo acessível à plataforma.</p>
            </div>
            <Link href="/" className={`inline-flex items-center justify-center px-4 py-2 font-bold transition-colors rounded-lg border-2 focus:outline-none focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-transparent border-yellow-300 text-yellow-300 hover:bg-yellow-300 hover:text-black' : 'bg-gray-100 border-transparent text-blue-700 hover:bg-gray-200 hover:text-blue-800'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col">
              <label htmlFor="tag" className={`mb-2 text-lg font-bold ${altoContraste ? 'text-yellow-300' : 'text-gray-800'}`}>Categoria do Curso <span aria-hidden="true" className="text-red-600">*</span></label>
              <select 
                id="tag" required aria-required="true"
                className={`p-4 text-lg transition-all border-2 rounded-lg outline-none cursor-pointer focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-black border-yellow-300 text-yellow-300 focus:border-yellow-400' : 'bg-white border-gray-300 focus:border-blue-700'}`}
                value={formData.tag} 
            ...
            {formData.tag === 'Outro' && (
              <div className="flex flex-col duration-300 animate-in fade-in slide-in-from-top-2">
                <label htmlFor="tagPersonalizada" className={`mb-2 text-lg font-bold ${altoContraste ? 'text-yellow-300' : 'text-gray-800'}`}>Digite a nova categoria <span aria-hidden="true" className="text-red-600">*</span></label>
                <input 
                  id="tagPersonalizada" type="text" required aria-required="true"
                  placeholder="Ex: Marketing, Gestão..."
                  className={`p-4 text-lg transition-all border-2 rounded-lg outline-none focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-black border-yellow-300 text-yellow-300 placeholder-yellow-600 focus:border-yellow-400' : 'bg-blue-50 border-blue-400 focus:border-blue-700'}`}
            ...
            <div className="flex flex-col">
              <label htmlFor="titulo" className={`mb-2 text-lg font-bold ${altoContraste ? 'text-yellow-300' : 'text-gray-800'}`}>Título do Curso <span aria-hidden="true" className="text-red-600">*</span></label>
              <input id="titulo" type="text" required placeholder="Ex: Introdução à Lógica com Python" className={`p-4 text-lg transition-all border-2 rounded-lg outline-none focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-black border-yellow-300 text-yellow-300 placeholder-yellow-600 focus:border-yellow-400' : 'bg-white border-gray-300 focus:border-blue-700'}`} value={formData.titulo} onChange={(e) => setFormData({...formData, titulo: e.target.value})} />
            </div>

            <div className="flex flex-col">
              <label htmlFor="descricao" className={`mb-2 text-lg font-bold ${altoContraste ? 'text-yellow-300' : 'text-gray-800'}`}>Breve Descrição <span aria-hidden="true" className="text-red-600">*</span></label>
              <textarea id="descricao" required rows={3} placeholder="Descreva o que será ensinado neste curso..." className={`p-4 text-lg transition-all border-2 rounded-lg outline-none resize-y focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-black border-yellow-300 text-yellow-300 placeholder-yellow-600 focus:border-yellow-400' : 'bg-white border-gray-300 focus:border-blue-700'}`} value={formData.descricao} onChange={(e) => setFormData({...formData, descricao: e.target.value})}></textarea>
            </div>

            <div className="flex flex-col">
              <label htmlFor="link" className={`mb-2 text-lg font-bold ${altoContraste ? 'text-yellow-300' : 'text-gray-800'}`}>Link do Vídeo no YouTube <span aria-hidden="true" className="text-red-600">*</span></label>
              <input id="link" type="url" required placeholder="https://www.youtube.com/watch?v=..." className={`p-4 text-lg transition-all border-2 rounded-lg outline-none focus:ring-4 focus:ring-orange-500 ${altoContraste ? 'bg-black border-yellow-300 text-yellow-300 placeholder-yellow-600 focus:border-yellow-400' : 'bg-white border-gray-300 focus:border-blue-700'}`} value={formData.link} onChange={(e) => setFormData({...formData, link: e.target.value})} />
            </div>

            <button type="submit" className={`flex items-center justify-center w-full py-4 mt-4 text-xl font-extrabold transition-colors rounded-lg shadow-lg outline-none focus:ring-4 focus:ring-orange-500 hover:shadow-xl ${altoContraste ? 'bg-yellow-300 text-black hover:bg-yellow-400' : 'bg-blue-700 text-white hover:bg-blue-800'}`}>
